...
class TablaEquipos:
    def __init__(self, root, div) -> None:
        ...
        self.table.heading("c1", text="Dueño")
        self.table.heading("c2", text="Nombre")
        self.table.heading("c3", text="Media")
    ...
    def my_display(self, n, division):
        ...
        gen = equipos().get_equipos_division(division, n, self.limit)
        next_gen = equipos().get_equipos_division(division, n + self.limit, self.limit)
        for item in self.table.get_children():
            self.table.delete(item)
        for team in gen:
            self.table.insert(
                parent="", index=tk.END, values=(team[0], team[1], int(team[2]))
            )
